...
let s:opfirst = '^' . get(g:,'kotlin_opfirst',
	  \ '\%([<>=,?^%|*/&]\|\([-.:+]\)\1\@!\|!=\|in\%(stanceof\)\=\>\)')
let s:continuation = get(g:,'kotlin_continuation',
...
function GetKotlinIndent()
  let b:kt_cache = get(b:,'kt_cache',[0,0,0])
  ...
  elseif syns =~? s:syng_str && l:line !~ '^[''"]'
	if b:kt_cache[0] == v:lnum - 1 && s:Balanced(v:lnum-1)
	  let b:kt_cache[0] = v:lnum
	endif
	return -1
  endif
  ...
  if b:kt_cache[0] >= l:lnum && b:kt_cache[0] < v:lnum &&
		\ (b:kt_cache[0] > l:lnum || s:Balanced(l:lnum))
	call call('cursor',b:kt_cache[1:])
  else
  ...
  let b:kt_cache = [v:lnum] + (line('.') == v:lnum ? [0,0] : getpos('.')[1:2])
  let num = b:kt_cache[1]
  ...
  if !num || s:IsBlock()
	...
	if idx < 0 && pline !~ '[{;]$'
	  if pline =~# ':\@<!:$'
		...
		let isOp = s:tern_col(b:kt_cache[1:2]) * s:W
	  else
		let isOp = (l:line =~# s:opfirst || s:continues(l:lnum,pline)) * s:W
	  endif
	  let bL = s:iscontOne(l:lnum,b:kt_cache[1],isOp)
